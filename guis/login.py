...
class Ui_Login(QtGui.QDialog):
	def __init__(self):
		...
		# self.dbu = db.DatabaseUtility(filename = 'krish.db', tableName = 'login')
		self.dbu = db.DatabaseForAll(tablename = 'login_table')
		self.setupUi(self)
		self.confirm = None
	...
	def retranslateUi(self, Login_Dialog):
		Login_Dialog.setWindowTitle(_translate("Login_Dialog", "Auto-life", None))
		self.groupBox.setTitle(_translate("Login_Dialog", "Login page", None))
		...
		for col in t:
			if username == col[1]: 
				if password == col[2]:
					...
					QtGui.QMessageBox.information(self,'hell no', 'rejected')
					break
		else:
			QtGui.QMessageBox.information(self, 'BOOYA!', 'failed!!')



	def openMain(self):
		homepage = mainwindow.Ui_MainWindow(self)
	...
	def print_message(self):
		...
		#QtGui.QMessageBox.information(self, 'BOOYA!', 'Success!!')
			#		self.close()
			#	else:
			#		QtGui.QMessageBox.warning(self, 'damm it', 'wrong pass')
			#else:
			#	QtGui.QMessageBox.warning(self, 'damm it', 'wrong username')
...
class Ui_Register(QtGui.QDialog):
	...
	def retranslateUi(self, Register_Dialog):
		...
		self.groupBox.setTitle(_translate("Register_Dialog", "Sign Up Page", None))
	...
	def Add_btn(self):
		...
		if not username:
			...
			for col in t:
				...
				self.dbu.AddToTable(username = username, password = password)
